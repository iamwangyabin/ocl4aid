...
class RineSideGaussTests(unittest.TestCase):
    def test_extracts_all_block_cls_tokens_and_scores_gaussian_head(self):
        torch.manual_seed(0)
        model = RineSideGauss(
        ...
        self.assertEqual(z.shape, (4, model.depth * model.embed_dim))
        self.assertEqual(model.feature_dim, model.depth * model.embed_dim)
